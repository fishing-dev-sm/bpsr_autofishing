...
# ====== A/D按键判定参数 ======
POINT_A_POS = (794, 540)           # 检查“A”键的判定点（窗口内坐标）
POINT_D_POS = (1124, 540)           # 检查“D”键的判定点（窗口内坐标）
...
# ====== 新版遛鱼方向（A/D）红橙色检测参数 ======
# 中心点（以 1920×1080 为基准坐标）
AD_A_CENTER = (794, 540)           # A 区域中心（新版规则）
AD_D_CENTER = (1124, 540)          # D 区域中心（新版规则）
AD_BOX_SIZE = 20                   # A/D 判定方框边长（像素，基于 1920×1080 缩放）
AD_RED_ORANGE_RATIO = 0.10         # ROI 内红/橙像素占比阈值（达到即触发）
...
# ====== 收鱼线红白色检测参数 ======
REEL_RED_CHECK_CENTER = (1236, 898)    # 收鱼线红白检测区域中心（基于1920x1080）
REEL_RED_CHECK_SIZE = 30                    # 检测区域大小（30x30像素）
REEL_RED_RATIO_THRESHOLD = 0.01             # 红色/白色像素占比阈值（宽泛检测）
REEL_CLICK_INTERVAL = 0.05                  # 鼠标连点间隔（秒）
REEL_WHITE_COLOR_TARGET = (253, 246, 246)   # 目标白色RGB值（基于实际截图#fdf6f6）
REEL_WHITE_COLOR_TOLERANCE = 25             # 白色检测容差（RGB颜色距离）
